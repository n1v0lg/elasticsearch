...
clusters. To use {ccr} and {ccs} safely, <<remote-clusters-security,configure remote cluster security>>.

//When connecting a local cluster to remote clusters, an {es} superuser
